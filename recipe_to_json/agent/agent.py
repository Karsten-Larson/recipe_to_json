...
async def fetch_node(state: GraphState):
    ...
    if validators.url(source):
        ...
        try:
            ...
            print(f"--- Failed to read file {source}: {e} ---")
            return {"content_bytes": None}

    return {"content_bytes": content_bytes}


def convert_node(state: GraphState):
    if not state.content_bytes:
        print("--- No content bytes to convert ---")
        return {"markdown_content": None}

    md = MarkItDown()

    try:
        result = md.convert(BytesIO(state.content_bytes))
...
def extraction_node(state: GraphState):
    ...
    if not content:
        print(f"--- No markdown content to extract from {state.input_source} ---")
...
def save_node(state: GraphState):
    ...
    if not recipe:
        print(f"--- No recipe to save for {state.input_source} ---")
        return {}
...
def should_proceed_to(node: str, state: GraphState):
    if node == "convert":
        return node if state.content_bytes is not None else END
    else:
        return node if state.markdown_content is not None else END

should_proceed_to_convert = partial(should_proceed_to, "convert")
...
workflow.add_conditional_edges("fetch", should_proceed_to_convert)
workflow.add_conditional_edges("convert", should_proceed_to_extract)
